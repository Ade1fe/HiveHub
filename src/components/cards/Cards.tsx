...
const Cards = ({ posts, onItemClick }: {posts: any, onItemClick: (postId: string) => void}) => {
  return (
    <Box className="texts" my='4' >
        {posts.map((post: any) => (
            <Box key={post.id} onClick={() => onItemClick(post.id)} borderRadius='20px' overflow='hidden' display={['flex']} w='' shadow='base' _hover={{ bg: "#222", color: "white", transform: "scale(1.009)", transition: "transform 0.3s ease-in-out" , borderRadius: "20px" }} cursor='pointer' alignItems='center' gap='4' >
                <Box w={['70%']} px={['20px']} py='40px'>
                    <Box display="flex" gap='4'>
                        ...
                        <Text color='black'>{post.timestamp ? post.timestamp.toLocaleDateString() + " " + post.timestamp.toLocaleTimeString() : ''}</Text>
                    </Box>
